...
class ArcFaceLoss(nn.Module):
    ...
    def forward(self, embeddings: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
      w_scaled = torch.div(self.W, torch.norm(self.W, p=2, dim=1, keepdim=True))
      emb_scaled = torch.div(embeddings, torch.norm(embeddings, p=2, dim=1, keepdim=True))
...
class AdaFaceLoss(nn.Module):
    def __init__(self, num_classes: int, embedding_dim: int = 512, m: float = 0.4, s: float = 64.0, t_alpha: float = 1.0):
      super().__init__()
      self.m = m
      self.s = s
    ...
    def forward(self, embeddings: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
      ...
      with torch.no_grad():
        ...
        std = safe_norm.std().detach()
        self.running_mean = mean * self.t_alpha + (1 - self.t_alpha) * self.running_mean
        self.running_std = std * self.t_alpha + (1 - self.t_alpha) * self.running_std

      embeddings = torch.div(embeddings, z_normed)
      # scale feature
      z_norm_hat = (z_normed - self.running_mean) / (self.running_std+self.eps)
      z_norm_hat = (z_norm_hat * self.h).clip(-1, 1)

      # norm weight
      weight_norm = torch.div(self.W, torch.norm(self.W, dim=1, keepdim=True))
